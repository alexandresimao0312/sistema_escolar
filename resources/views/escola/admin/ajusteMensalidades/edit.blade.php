...
@endif
  <!-- Fim do Alert -->
  <form action="{{ route('admin.ajustes.update', $ajuste->id) }}" method="POST">
    @csrf
    @method('PUT')
    <div class="mb-3">
        ...
        <select name="curso_id" id="curso_id" class="form-control" required>
            @foreach ($cursos as $curso)
                <option value="{{ $curso->id }}" {{ $ajuste->curso_id == $curso->id ? 'selected' : '' }}>
                    {{ $curso->nome }}
                </option>
            @endforeach
        </select>
    ...
    <div class="mb-3">
        ...
        <input type="number" name="ajuste" id="valor_mensalidade" class="form-control" required value="{{ $ajuste->valor_mensalidade }}" step="0.01" min="0">
    </div>
